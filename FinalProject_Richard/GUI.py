...
class RotationCurves(tk.Tk): 
	def __init__(self, *args, **kwargs):
		...
		fileMenu.add_command(label="New Window", command= lambda: combine_funcs( NewWindow()))
...
def NewWindow():
	...
	if counter == 1:
			NW = tk.Toplevel()
			NW.wm_title("Rotation Curves Client")
			Instruction = tk.Label(NW, text = 'Enter Values', font = LARGE_FONT).grid(row = 0, column = 3, sticky = W)
			Redshift = tk.Label(NW, text = "Redshift:").grid(row = 1, column = 0, columnspan = 1,  sticky = W)
			RedshiftBlank = Entry(NW)
			RedshiftBlank.grid(row = 1, column = 3, columnspan = 1, sticky = W)
			AVGWL = tk.Label(NW, text = "Average Wavelength:").grid(row = 2, column = 0, columnspan = 1, sticky = W)
			AVGWLBlank = Entry(NW)
			AVGWLBlank.grid(row =2 , column = 3, columnspan = 1, sticky = W)
			ChangeWl= tk.Label(NW, text = "Change in Wavelength").grid(row = 3, column = 0, sticky = W)
			ChangeWLBlank = Entry(NW)
			ChangeWLBlank.grid(row = 3, column = 3, columnspan = 1, sticky = W)
			Distance = Label(NW, text = "Vertical Distance").grid(row = 4, column = 0, sticky = W)
			DistanceBlank = Entry(NW)
			DistanceBlank.grid(row = 4, column = 3, columnspan = 1, sticky = W)
			start_Solve = tk.Button(NW, text = 'Solve', command = lambda: Solve()).grid(row = 5, column = 3, sticky = W) #This button runs the rotation curve solver #Where the button is on the GUI		#button1.grid(row = 5, column = 2, sticky = W)
			start_Graph = tk.Button(NW, text = 'Graph', command = lambda: combine_funcs(Solve())).grid(row = 5, column = 3, padx = 125, sticky = W) #Graphs the curve on a different grame. Second command brings the frame forward.
			start_Exit = tk.Button(NW, text = "Exit", command = lambda: sure()).grid(row = 6, column = 3, padx = 70, sticky = W)
			def sure():
				sure = tk.Toplevel()
				sure.wm_title("Exit")
				check = tk.Label(sure, text = "Are you sure?", font = LARGE_FONT).grid(row = 0, column = 2, columnspan = 2, sticky = W)
				sure.sure_yes = tk.Button(sure, text = "Yes", command = combine_funcs(sure.destroy, NW.destroy)).grid(row = 2, column = 3, sticky = W)
				sure_no = tk.Button(sure, text = "No", command = sure.destroy).grid(row = 2, column = 4, sticky =W)
			def Solve(): #Popups when pressing solve
				...
				def graph1():	
					if effect_radius is 'Error':
						...
						b = g.add_subplot(111)
						g.suptitle("Velocity versus Radius", fontsize = 14)
						b.set_xlabel('Radius')
						b.set_ylabel('Velocity')
						x = effect_radius
						y = effect_velocity
						b.plot(x,y)
						b.axis([0, 10000000, 0, 10000000000])
						canvas = FigureCanvasTkAgg(f, NW)
						canvas.show()
		...
		def Solve(): #Popups when pressing solve
			...
			def graph1():	
				if effect_radius is 'Error':
					...
					a = f.add_subplot(111)
					f.suptitle("Velocity versus Radius", fontsize = 14)
					a.set_xlabel('Radius')
					...
					canvas.show()
					canvas.get_tk_widget().grid(row = 15, column = 0, rowspan = 50, columnspan = 50, sticky =W)
